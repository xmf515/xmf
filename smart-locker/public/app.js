...
function initializeApp() {
  console.log('初始化检查:', {
    Paho: typeof Paho,
    windowPaho: typeof window.Paho,
    Client: typeof window.Paho?.MQTT?.Client
  });

  if (!window.Paho?.MQTT?.Client) {
    console.error('Paho.MQTT.Client 不可用');
    updateStatus("disconnected", "MQTT库初始化失败");
...
function connectToServer() {
  const Paho = window.Paho; // 确保使用全局对象
  
  const server = CONFIG.servers[currentServerIndex];
  console.log('尝试连接:', server.host, '使用Paho版本:', Paho.VERSION);
  ...
  try {
    mqttClient = new Paho.MQTT.Client(
      ...
      Number(server.port),
      server.path,
      "web_" + Math.random().toString(36).substr(2, 8)
    );
    ...
    console.error('创建客户端异常:', e);
    updateStatus("disconnected", "连接初始化失败");
    tryNextServer();
  }
}
